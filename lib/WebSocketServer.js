...
function WebSocketServer(options, callback) {
  ...
  if (options.value.port) {
    var httpObj = options.value.httpp ? httpp : http;
    ...
    // check if WebSocketServer server
    this._server = (options.value.server instanceof WebSocketServer) ? 
                    options.value.server._server : options.value.server;
